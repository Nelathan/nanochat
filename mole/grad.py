...
class MoleGradFn(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor, weight: torch.Tensor, AQ: torch.Tensor, mole_Y: torch.Tensor) -> torch.Tensor:
        # AQ is pre-computed in MoleLinear to allow fusion with Z-sampling
        ctx.save_for_backward(weight, AQ, mole_Y)
        return F.linear(x, weight, None)

    @staticmethod
    @custom_bwd(device_type="cuda")
    def backward(ctx, grad_output: torch.Tensor):
        weight, AQ, mole_Y = ctx.saved_tensors
        ...
        torch.addmm(input=mole_Y, mat1=grad_output_flat.T, mat2=AQ, beta=1.0, alpha=1.0, out=mole_Y)
